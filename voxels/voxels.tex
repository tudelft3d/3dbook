...
Voxel models, which are the 3D equivalent of 2D rasters, are a common way to store 3D models of the built environment using a regular 3D grid (Figures~\ref{fig:voxel}, ~\ref{fig:voxelmodel} and~\ref{fig:model}).
...
\begin{marginfigure}
  \centering
  \includegraphics[width=\textwidth]{figs/voxel}
  \caption[A voxel]{A voxel is a box-shaped element (cuboid), equivalent to a pixel in 2D.}% 
\label{fig:voxel}
\end{marginfigure}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figs/voxelmodel}
  \caption[A voxel model]{A voxel model is a representation of an object or field comprised of voxels.}% 
\label{fig:voxelmodel}
\end{figure}
\begin{figure}[htbp]
\centering
...
\item \textbf{defining the shape of a domain}\marginnote{voxel domain}\index{voxel domain} (Figure~\ref{subfig:domain}) in which the objects to be represented fit, or alternatively in which the region of interest of a field fits, \eg\ a bounding box defined by their minimum and maximum coordinates along each axis;
\begin{figure}[htbp]
\centering
\begin{subfigure}[b]{\linewidth}
\includegraphics[width=\linewidth]{figs/domain}
\caption{}%
\label{subfig:domain}
\end{subfigure}
\\
\begin{subfigure}[b]{\linewidth}
\includegraphics[width=\linewidth]{figs/dividing}
\caption{}%
\label{subfig:dividing}
\end{subfigure}
\caption[Defining and dividing the domain of a voxel model]{(a) Defining and (b) dividing the domain of a voxel model}%
\label{fig:domain}
\end{figure}

\item \textbf{dividing the domain} (Figure~\ref{subfig:dividing}) using a structure of many \emph{cells}\marginnote{voxel cells}\index{voxel cells}, usually following a regular or semi-regular pattern that can be defined programmatically (as opposed to explicitly representing the shape of each individual cell), \eg\ a grid defined by the number of cells along each axis;
...
Among the variations of voxel grids, \emph{sparse voxel models}\marginnote{sparse voxel model}\index{sparse voxel model} are used widely in practice to describe objects (\ie\ not fields) and are thus worth describing in more detail here.
...
Another potential way to compress the space used in voxel models is to use \emph{run-length encoding}\marginnote{run-length encoding}\index{run-length encoding} (RLE), which is applied after defining the order of the voxel domain.
In it, consecutive identical values of voxels are replaced by a single value and a count.
Since datasets of the built environment tend to have large contiguous empty regions, RLE often achieves very high compression ratios.
The main trade-off is that random access to an individual voxel can require scanning the run-length list.

More sophisticated compression methods---such as differential encoding (storing only differences between adjacent voxels) or block-based lossy compression---can achieve even higher ratios, particularly for field data where nearby values are correlated.
These are not yet widely used for 3D geospatial data but are common in other fields (eg medical imaging).
Note that sparse voxel models and hierarchical subdivisions (discussed below) are also forms of compression that avoid storing empty regions in the first place.
...
\item[octrees]\marginnote{octree}\index{octree} subdivide a node's space evenly along the \(x\), \(y\) and \(z\) axes into eight equal-size \emph{octants}\marginnote{octant}\index{octant}.
They are the 3D equivalent of quadtrees in 2D, which subdivide along the \(x\) and \(y\) axes into four equal-size quadrants.
Octrees are typically built top-down, starting from a root node covering the entire domain and recursively subdividing nodes based on a splitting predicate---such as containing more than a threshold number of objects or containing an object's boundary.
Usually, an additional termination condition is set (\eg\ maximum depth or minimum node size) to avoid the creation of potentially infinite levels.
This adaptivity makes octrees well-suited for scenes with large homogeneous regions, such as empty airspace above a city model.

The two main implementation variants are \emph{pointer-based octrees}, where each node stores links (pointers or IDs) to its eight children, and \emph{linear octrees}\index{linear octree}, where nodes are all stored in the same array and indexed using the unique long ID that is given by a space-filling curve (eg a Morton code) that encodes the path from root to node.
\begin{marginfigure}
  \centering
  \includegraphics[width=\textwidth]{figs/morton}
  \caption[The Morton code can uniquely identify octree octants]{The Morton (or Z-order) space-filling curve can be used to create a code that uniquely identifies each octant in an octree. From Wikimedia Commons.}% 
\label{fig:morton}
\end{marginfigure}
Linear octrees are a bit more difficult to implement, but they are more compact and can simplify implementing operations on them (eg as Boolean set union and intersection) because the IDs uniquely identify nodes (even when they are in different octrees of the same domain).
A notable hybrid is the \emph{sparse voxel octree}\index{sparse voxel octree} (SVO), which stores only occupied nodes within an octree framework, combining the memory efficiency of sparse voxel models with the adaptive resolution of an octree.
SVOs are widely used in real-time graphics for GPU-based ray tracing and level-of-detail rendering.

\item[bintrees]\marginnote{bintree}\index{bintree} are similar to octrees, but subdivide space in halves along only one axis per node, switching to a different axis at each level of the tree, producing binary trees with exactly two children per node.
The most common axis ordering cycles through \(x\), then \(y\), then \(z\), then \(x\) again, and so on.
An adaptive strategy that splits along the longest axis of the node can also used, eg when the domain is extended more along some axes; or when objects have anisotropic shapes, eg when there is more detailed geometry along a certain axis.

Because each node has only two children instead of eight, bintrees use less memory per node for child links.
The trade-off is that they generally produce deeper trees than octrees for the same scene, which can increase query time in the worst case.
Bintrees are particularly effective for fields with anisotropic resolution requirements---for instance, a point cloud that is densely sampled in the horizontal plane but sparsely in the vertical direction, or a terrain with much greater extent in \(x\) and \(y\) than in \(z\).

\item[\(k\)-d trees]\marginnote{k-d tree}\index{k-d tree} are similar to bintrees in that each node splits space into two children, but the splitting plane can be positioned arbitrarily along the chosen axis, not necessarily at the midpoint.
The split coordinate is chosen using different strategies: the \emph{median} of the data in the node (producing a balanced tree with equal-sized children), the \emph{midpoint} of the node's extent (simpler but possibly unbalanced), or a location that separates objects into disjoint sets (ie no objects are crossed at that specific value).

K-d trees are most commonly used in practice for organising unstructured point clouds rather than for volumetric representations.
Their efficient nearest-neighbour and range query performance (ie obtaining the objects in a given range) makes them a standard tool in point cloud processing, and they are widely implemented in libraries such as nanoflann and CGAL.
When used for volumetric representation, k-d trees can achieve higher compactness than octrees by placing split planes at object boundaries, but this comes at the cost of more complex operations.
...
\includegraphics[width=\linewidth]{figs/simplification-1}
\caption{}%
\label{subfig:simplification-1}
\end{subfigure}%
\begin{subfigure}[b]{0.33\linewidth}
\includegraphics[width=\linewidth]{figs/simplification-2}
\caption{}%
\label{subfig:simplification-2}
\end{subfigure}%
\begin{subfigure}[b]{0.33\linewidth}
\includegraphics[width=\linewidth]{figs/simplification-3}
\caption{}%
\label{subfig:simplification-3}
\end{subfigure}
\caption[Decimation of a mesh]{(a) Starting from a 2.5D raster terrain (or full 3D voxel model), (b) we can obtain a 2.5D or 3D b-rep mesh by creating triangles or quads, and (c) decimate it to obtain a simplified vector model.}%
