...
For instance, a user of 3D modelling or design software will find it easier to adjust the shape of a curved surface based on a few key points than to adjust the vertices of many small triangles to achieve a similar result.
...
Among the possible methods that can be used to represent curves in 2D/3D and curved surfaces in 3D, this chapter covers two related methods that are often found in practice: B\'ezier curves/surfaces and B-splines.
...
\subsection{Segments and patches}

When we want to model a curve or curved surface with a complex shape consisting of many bends and turns, it is often easier to split it into \emph{segments}\marginnote{curve segment}\index{curve segment} (for curves) and \emph{patches}\marginnote{surface patch}\index{surface patch} (for surfaces).
In this way, we can model each part independently and get more precise results.

\subsection{Uniform vs.\ non-uniform}

In a parametric curve \(C(t)\), specific values of \(t\) represent points that are used to split the curve into segments.
...
\caption{A composite B\'ezier curve made from three segments. In B\'ezier curves, the values of \(t\) are evenly spaced, and it is therefore a uniform parametric curve.}%
\label{fig:segments}
\end{figure*}
...
In other words, even if we `move' from \(t = 0\) to \(t = 1\) at a constant speed, we will likely not move at the same speed across the curve or surface.
...
Spline curves are named after \emph{flat splines}\marginnote{flat spline}\index{flat spline}, which were thin flexible strips of wood or metal used to draw smooth curves through a set of points before the advent of computers (Figure~\ref{fig:spline}).
...
A b-spline curve is a parametric curve created based on two kinds of elements: (i) a sequence of points that are used by all the segments in the curve, and (ii) a sequence of non-decreasing values called \emph{knots}\marginnote{knot}\index{knot}, which define how the parameters in the parametric curve are evaluated.
The former are similar to the data and control points used in B\'ezier curves.
The latter are the values of the parameter that split the curve into segments, which in the case of a B\'ezier curve were simply 0 and 1.
...
\subsection{Why not just use polynomials?}

Polynomial functions \emph{can} be used to directly model entire curves and surfaces.
