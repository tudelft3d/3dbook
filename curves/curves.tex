...
This is easiest to see when we consider that the data or control points are not evenly spaced either---we will move slower through parts with bunched up points, but it can also happen even when the points are evenly spaced.
...
A b-spline curve is a non-uniform parametric curve created based on two kinds of elements: (i) a sequence of control points that are used by all the segments in the curve, and (ii) a sequence of non-decreasing values called \emph{knots}\marginnote{knot}\index{knot}, which define how the parameters in the parametric curve are evaluated.
The former are similar to the data and control points used in B\'ezier curves.
The latter are the values of the parameter that split the curve into segments, which in the case of a (uniform) simple B\'ezier curve were simply 0 and 1.

Given a sequence of \(n+1\) points \((p_0, \ldots, p_n)\), a b-spline of order \(k\) is composed of segments, each of which is created based on a sub-sequence of \(k\) control points from the curve in a moving window.

Since b-splines are created from a limited number of control points, they have \emph{local control}\marginnote{local control}\index{local control}, meaning that moving a control point affects only the segments of the curve that use that control point.
This is a major advantage over B\'ezier curves, where every control point influences the entire curve.
Continuity in b-splines is also easy to achieve, with a b-spline of degree \(n\) being able to have up to \(C^{n-1}\) continuity at its knots.

A b-spline curve 
\(S\)


The b-spline basis functions can be defined recursively using the Cox--de Boor recursion formula\marginnote{Cox--de Boor recursion}\index{Cox--de Boor recursion}:

\begin{equation}
\label{eq:bsplinebasis0}
...
\begin{equation}
\label{eq:bsplinebasis}
N_{i,p}(t) = \frac{t - k_i}{k_{i+p} - k_i} N_{i,p-1}(t) + \frac{k_{i+p+1} - t}{k_{i+p+1} - k_{i+1}} N_{i+1,p-1}(t)
\end{equation}

where \(k_i\) are the knots and \(p\) is the degree of the curve.
A b-spline curve of degree \(p\) is then defined as:

\begin{equation}
\label{eq:bsplinecurve}
C(t) = \sum_{i=0}^n N_{i,p}(t) p_i
\end{equation}

In \emph{uniform} b-splines\marginnote{uniform b-spline}\index{uniform b-spline}, the knot values are evenly spaced, \eg\ \((0, 1, 2, 3, 4, 5)\), and the basis functions for each segment are identical up to translation.
