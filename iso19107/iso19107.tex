...
This document gives an overview of the primitives in 3D, both from the ISO19107 and the GML points of view.
...
The geometric primitives as used in 3D GIS are based on the ISO19107 definitions, and the definition of a polyhedron there is broader than that of a 2-manifold, to allow us to represent all the real-world features.

As shown in Figure~\ref{fig:isoprimitives}, the ISO19107 geometric primitives for representing an object are: a 0D primitive is a \texttt{GM\_Point}, a 1D a \texttt{GM\_Curve}, a 2D a \texttt{GM\_Surface}, and a 3D a \texttt{GM\_Solid}.
\begin{marginfigure}
  \centering
  \includegraphics[width=\linewidth]{figs/isoprimitives.pdf}
  \caption{ISO 19107 primitives relevant for the modelling of the built environment.}%
...
[\ldots] 
The \texttt{GM\_OrientableSurfaces} that bound a solid shall be oriented outward.
\end{definition}
\begin{definition}
...
\begin{marginfigure}
  ...
  \caption[A `squared torus']{A `squared torus' is modelled with one exterior boundary formed of ten surfaces. Notice that the solid has no interior boundaries.}%
\label{fig:torus}
\end{marginfigure}
...
GML (and CityGML) has classes for each dimensionality (\texttt{Multi*}); the most relevant one in our context is \texttt{MultiSurface}, which is often used in practice to represent the geometry of a building.
...
The third assertion means that the boundaries of shells can intersect each other, but the intersection between the shells can only contain primitives of dimensionality 0 (vertices) and 1 (edges).
...
If the interior of the two interior shells intersects (as in $s_4$) the solid is not valid; this is also related to the sixth assertion stating that each cavity must define one connected component: if the interiors of two cavities are intersecting they define the same connected component.
...
In 3D, if one polygon is used to construct a shell, its exterior ring must be oriented in such a way that when viewed from outside the shell the points are ordered counter-clockwise.
...
For details on how the validation of the 3D primitives can be implemented, see \citet{13_cacaie} and \citet{18_ogdss_val3dity}.
...
\begin{enumerate}
  ...
  \item In which direction does the normal of the interior shell of the solid point to in Figure~\ref{fig:geomprimitives}?
\end{enumerate}
