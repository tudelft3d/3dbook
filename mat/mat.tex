...
It can be considered a \emph{dual} representation to the b-rep (\refchap{chap:brep}), similar to how the Voronoi diagram is dual to the Delaunay triangulation (\refchap{chap:dtvd3d}). 
...
For DSMs for example, equally meaningful decompositions into parts can be made, \eg\ the MAT allows us to decompose a DSM into separate hills, watercourses and other objects on top of the DSM (see Figure~\ref{fig:matterrain}).
...
Harry Blum, who originally introduced the MAT, finally settled on symmetry axis, as he considered symmetry to be the crucial role of the MAT \citep{Blum73}.} that apply. 
...
The medial branches, its junctions and how those are connected define the \emph{medial structure}\marginnote{medial structure}\index{medial structure}.
...
For a 2D object, such as in Figure~\ref{fig:gingerman}, the medial branches are curves and the junctions are points. 
...
\subsection{Exterior MAT and the MAT of a DSM}
...
The separation between inside and outside is very clear and unambiguous for an object with a closed boundary such as the gingerbread man of Figure~\ref{fig:gingerman} or for any perfectly manifold boundary.
...
For example for a DSM we can follow the convention that the `ground side' of the DSM is the interior, and the `sky side' is the exterior, as follows from Figure~\ref{fig:object_earth}.
\begin{figure*}
	\centering
	...
	\begin{subfigure}{0.57\linewidth}
		...
		\subcaption{For a terrain the MAT is typically subdivided into open clusters that correspond to features such as hills, buildings and watercourses in the terrain. Shown here is a vertical cross section of a DSM\@. Exterior MAT in light blue, interior MAT in dark blue.}%
		\label{fig:matterrain}
	\end{subfigure}
	\caption{Defining interior and exterior for an open surface such as a terrain.}
	\label{fig:intext}
\end{figure*}
Following this convention, we can still define the interior and exterior MAT of a DSM, see for instance Figure~\ref{fig:matterrain}.
...
For objects with open boundaries like DSMs, there can also be multiple interior medial clusters.
...
\subsection{Structuring the MAT}
To use the MAT in practice, its medial atoms, branches and sheets must be organised into a data structure that captures both their geometry and their connectivity at junctions.

In 2D, the MAT has a simple graph structure: the medial branches are edges, and the junctions and branch tips are nodes.
Each node stores its position and radius, and edges can be assumed to be linear geometries between nodes, which results in a linear interpolation of both the node geometries and radii.
A more complex representation could use curves, eg a parametric curve in each edge and a radius function of its branch.

In 3D, the MAT is a cell complex with three types of cells:
\begin{description}
	\item[Point junctions] (0-cells) where three or more curve junctions meet or a curve junction ends.
	\item[Curve junctions] (1-cells) where three or more medial sheets meet.
	\item[Medial sheets] (2-cells) bounded by curve junctions and point junctions.
\end{description}
As in 2D, we could opt for a simple implementation with linear geometries or having sheets that stores their surface geometry and a radius function over the sheet.

All of these implementations could be stored using the b-rep data structures described in \refchap{chap:brep}.
...
\subsection{MAT approximation}\label{sec:mat-approximation}
The MAT can be approximated in various ways, \eg\ by using voxels (\refchap{chap:voxels}) and distance transforms or as a subset of the Voronoi diagram (\refchap{chap:dtvd3d}).
However, here we will focus on the so-called shrinking-ball algorithm.

\subsubsection{The shrinking-ball algorithm}
The shrinking-ball algorithm works well for robustly approximating the MAT of 3D objects that are represented using boundary points, \ie\ point clouds (\refchap{chap:LoD2recon}).
...
The extra boundary point $\mathbf{q}$ that the ball must touch (besides $\mathbf{p}$) changes with each iteration. Together, $\mathbf{p}$ and $\mathbf{q}$ determine where along the normal line the next centre lies.
...
Intuitively, $\mathbf{q_{next}}$ is closer to the normal line than the current ball's surface, so the next ball must shrink: we slide the centre back along $\vec{\mathbf{n}}$ until it is equally far from $\mathbf{p}$ and $\mathbf{q_{next}}$. Since $\mathbf{c_{next}}$ must lie on $\mathbf{p} + t \vec{\mathbf{n}}$ and satisfy $|\mathbf{c_{next}} - \mathbf{p}| = |\mathbf{c_{next}} - \mathbf{q_{next}}|$, we solve for $t$:
\[
t = -\frac{|\mathbf{p} - \mathbf{q_{next}}|^2}{2 \vec{\mathbf{n}} \cdot (\mathbf{p} - \mathbf{q_{next}})},
\qquad
\mathbf{c_{next}} = \mathbf{p} + t \vec{\mathbf{n}},
\qquad
r_{next} = t.
\]
...
\begin{algorithm}[htbp]
	...
	\DontPrintSemicolon\Input{
		...
		its normal vector $\vec{\mathbf{n}}$, and\\
		the initial ball radius $r_{init}$}
	\Output{the medial ball centre $\mathbf{c}$,\\the medial ball radius $r$}
	$r \leftarrow r_{init}$ \\
	$\mathbf{c} \leftarrow$ centre of the ball that touches $\mathbf{p}$, is centred on $\vec{\mathbf{n}}$, and has radius $r$ \\
	\Repeat{a \textbf{break} statement is executed} {
		...
		$\mathbf{c_{next}} \leftarrow \mathbf{p} + t \vec{\mathbf{n}}$ where $t = -|\mathbf{p} - \mathbf{q_{next}}|^2 / (2 \vec{\mathbf{n}} \cdot (\mathbf{p} - \mathbf{q_{next}}))$ \\
		\If{$r_{next} = r$} {
			\textbf{break}
		}
...
% Region-growing can also be used to segment the MAT of a DSM in its different disjoint medial clusters.
...
\end{figure}
This is especially problematic when $\mathcal{B}$ has some noise as is the case with the typical DSM.
...
\begin{figure}
	...
	\begin{subfigure}{0.4\linewidth}
		...
		\caption{Reconstructed object for $\lambda=10$. Notice how the tree is separated from the ground and that edges have become rounder.}
		\label{fig:imaimp:f}
	\end{subfigure}
...
The Medial Axis Transform was originally introduced in 1967 by Harry Blum, a biologist \citep{Blum67}. He later elaborated on its properties and argued for symmetry as the central organising principle of the MAT \citep{Blum73}.

The MAT is closely related to the Voronoi diagram (\refchap{chap:dtvd3d}): for a dense set of boundary points, the medial axis can be approximated as a subset of the Voronoi diagram, and the medial balls as a subset of the Voronoi cells. More precisely, the interior MAT corresponds to the Voronoi vertices (and edges) that lie inside the object.

\citet{Ma12} introduced the shrinking ball algorithm as a practical method for approximating the MAT from point clouds. \citet{Peters18} explains how to make the algorithm robust so that it can be successfully applied to lidar point cloud inputs, and how to obtain a sheet and cluster segmentation using a region-growing approach. An application of this to the automatic identification of watercourses from point clouds is described by \citet{Broersen17}.

Since the MAT encodes both the position and the thickness of an object, it can be used to reconstruct the object boundary from a pruned skeleton. This makes it useful for shape simplification, where small-scale boundary noise is removed while preserving the overall shape. Applications of the MAT beyond the geospatial domain include path planning (the skeleton defines a collision-free routes), mesh generation, and shape animation.
