...
Consequently, combinatorial maps have no \(\beta_0\) operation; the lowest-indexed operation is \(\beta_1\).
...
For combinatorial maps this formula holds for \(i \ge 1\); for a vertex (\(i = 0\)) the orbit follows all \(\beta_1, \ldots, \beta_n\), since there is no \(\beta_0\).
