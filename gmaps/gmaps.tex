...
\item every simplex in an \(n\)D combinatorial map\marginnote{nD combinatorial map}\index{nD combinatorial map} has two zero-dimensional vertices and one vertex of every dimension from \(2\) up to \(n\) (\ie\ \(0, 0, 2, \ldots, n\)).
\end{itemize}
...
For an \(n\)-dimensional generalised map, we can denote this as \(\langle \alpha_0, \ldots, \alpha_{i-1}, \alpha_{i+1}, \ldots, \alpha_n \rangle(d)\), and for an \(n\)-dimensional combinatorial map, we can denote this as \(\langle \beta_1, \ldots, \beta_{i-1}, \beta_{i+1}, \ldots, \beta_n \rangle(d)\). For combinatorial maps this formula holds for \(i \ge 1\); for a vertex (\(i = 0\)) the orbit follows all \(\beta_1, \ldots, \beta_n\), since there is no \(\beta_0\).
...
\begin{enumerate}
	...
	\item Look at the differences between g-maps and c-maps. Why is implementing algorithms on c-maps often much harder than on g-maps? Think about how this relates to implementing algorithms on full edge-based data structures vs.\ half-edge data structures.
