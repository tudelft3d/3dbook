...
Building information modelling (BIM) involves the creation and use of detailed digital 3D models of buildings or infrastructure in a way that supports their design and construction, but increasingly also extending to their planning, operation, maintenance, refurbishing and/or demolition\marginnote{building information modelling}\index{building information modelling}\marginnote{BIM}\index{BIM}.
...
Relevant to this, some software vendors and organisations (\eg\ the American Institute of Architects and the UK BIM Task Group) have come up with the concept of the level of development (LOD)\marginnote{level of development}\index{level of development}\marginnote{LoD}\index{LoD} of a BIM model, which is equivalent to the concept of level of detail (also LOD or LoD) in GIS\@.
...
In this, practitioners and software vendors very often refer to 4D BIM\marginnote{4D BIM}\index{4D BIM}, which means that a model includes time information, generally in the context of construction scheduling.
Note however that this information might not be in a convenient format, \eg\ a linked Word file with a textual description.
Other dimension definitions sometimes refer to 5D BIM\marginnote{5D BIM}\index{5D BIM}, which usually means a model with cost information.
Finally, different people use 6D\marginnote{6D BIM}\index{6D BIM} or 7D BIM\marginnote{7D BIM}\index{7D BIM} to refer to various other aspects (sustainability, facility management, and delivery of as-built models among others), but the definitions for these are very inconsistent.
...
BIM is an industry-dominated field, and software-specific file formats are still the main way in which files are exchanged, such as using the native formats of Autodesk's Revit\marginnote{\faExternalLink\ \url{https://www.autodesk.com/products/revit/overview}} and Graphisoft's ArchiCAD\marginnote{\faExternalLink\ \url{https://www.graphisoft.com/archicad/}} (BIMx).
...
As a way to solve this problem, the buildingSMART consortium, which notably includes a number of software companies (including Autodesk and the Graphisoft-owning Nemetschek Group)\marginnote{\faExternalLink\ \url{https://www.buildingsmart.org/members/member-directory/}}, created the industry foundation classes\marginnote{industry foundation classes}\index{industry foundation classes} (IFC)\marginnote{IFC}\index{IFC}\marginnote{\faExternalLink\ \url{http://www.buildingsmart-tech.org/specifications/ifc-releases}} as an open data model for the exchange of BIM models.
...
They can contain many types of classes (130 defined types, 217 enumeration types, 60 select types, 816 entities, 47 functions, 2 rules, 415 property sets, 93 quantity sets and 1697 individual properties in IFC 4.2\marginnote{\faExternalLink\ \url{https://standards.buildingsmart.org/IFC/DEV/IFC4_2/FINAL/HTML/}}), which are defined using the EXPRESS\marginnote{EXPRESS}\index{EXPRESS} data modelling language.
Among others, there are several classes to model actors\marginnote{actor}\index{actor} (\eg\ people and organisations), controls\marginnote{control}\index{control} (\eg\ specifications, regulations, schedules and other requirements), processes\marginnote{process}\index{process} (\eg\ actions during construction), products\marginnote{product}\index{product} (\eg\ physical building elements and other spatially defined objects), the project itself (\eg\ where it is placed), and resources (\eg\ cost, materials and equipment), as well as groups of other classes (\eg\ those having a common purpose).
...
In particular, the Kernel\marginnote{IFC kernel}\index{IFC kernel} contains the root classes for the definition of objects, relationships and properties and their relationships (\eg\ \texttt{IfcRoot}, superclass of all the other entities; \texttt{IfcRelationship}, superclass of all relationships; \texttt{IfcObject}, which is the parent entity of \texttt{IfcGroup}, \texttt{IfcActor}, \texttt{IfcResource}, \texttt{Ifc\-Con\-trol}, \texttt{IfcProcess}, \texttt{IfcProject} and \texttt{IfcProduct}, being specified in the further extensions of the model).
...
Some even more specific information can be represented through the domain specific part of the schema, which can specify either classes represented in the interoperability layer\marginnote{IFC interoperability layer}\index{IFC interoperability layer} or in the product extension directly (\texttt{IfcArchitectureDomain}, \texttt{IfcBuildingControlsDomain}, \texttt{Ifc\-Cons\-truc\-tion\-Mgmt\-Do\-main}, \texttt{IfcElectricalDomain}, \texttt{IfcHvacDomain}, \texttt{Ifc\-Plum\-bing\-Fire\-Pro\-tec\-tion\-Do\-main}, \texttt{Ifc\-Struc\-tu\-ral\-A\-na\-ly\-sis\-Do\-main}, \texttt{Ifc\-Struc\-tu\-ral\-E\-le\-ments\-Do\-main}).

The resource layer\marginnote{IFC resource layer}\index{IFC resource layer} defines entities to further describe the objects defined in the other levels.

\subsection{Formats, encodings and federated models}

The most common encoding of IFC files is the STEP Physical File (SPF)\marginnote{STEP physical file}\index{STEP physical file}\marginnote{SPF}\index{SPF}, which is a plain text format that is reasonably compact, easy to parse by a computer and human readable (Figure~\ref{fig:step}).
...
In addition to STEP files, there is also an XML encoding of the standard (IFC-XML)\marginnote{IFC-XML}\index{IFC-XML} with file extension \texttt{.ifcXML}, as well as a zipped version of the other encodings with extension \texttt{.ifcZIP}\marginnote{IFC zip}\index{IFC zip}.
...
They are combined together in a federated model\marginnote{federated model}\index{federated model}.

\subsection{How objects are modelled}

Physical elements in IFC\marginnote{IFC element}\index{IFC element} (\ie\ \texttt{IfcElement}) are usually modelled separately using a local coordinate system that is defined per object (as opposed to the national or regional coordinate systems used in GIS).
...
In concrete terms, a product in IFC\marginnote{IFC product}\index{IFC product} (\ie\ \texttt{IfcProduct}, which is a superclass of \texttt{IfcElement}), is linked to a geometry (\ie\ \texttt{IfcProductRepresentation}\marginnote{IFC product representation}\index{IFC product representation}) and to the local coordinate system that defines its location (\ie\ \texttt{Ifc\-Ob\-ject\-Place\-ment})\marginnote{IFC object placement}\index{IFC object placement}.
...
The geometry of a physical element\marginnote{IFC geometry}\index{IFC geometry} can be created using a variety of representation paradigms:
\begin{description}
  ...
  IFC uses this paradigm to define various forms of 2D profiles\marginnote{IFC profile}\index{IFC profile} (Figure~\ref{fig:profiles}), as well as volumetric objects, such as spheres, cones and pyramids.
  ...
  \item[Sweep volumes:] a solid can also be defined by a 2D profile (a circle, a rectangle or an arbitrary polygon with or without holes) and a curve along which the surface is extruded\marginnote{IFC sweep volume}\index{IFC sweep volume} (Figure~\ref{fig:sweeps}).
...
\texttt{IfcOpening}\marginnote{IFC opening}\index{IFC opening} defines the 3D objects that are subtracted from another geometry (\eg\ a hole for a window in an \texttt{IfcWall} or for a staircase in an \texttt{IfcSlab}).
The \texttt{IfcOpening} can be in turn filled by an element, like an \texttt{IfcWindow} or \texttt{IfcDoor}.
Meanwhile, \texttt{IfcSpaces}\marginnote{IFC space}\index{IFC space} are used to explicitly model meaningful spaces (\eg\ rooms, apartments or storeys), and it is possible to associate attributes to them.
...
In order to represent objects which are not included in the IFC model, an \texttt{IfcProxy}\marginnote{IFC proxy}\index{IFC proxy} element is foreseen, which is a subclass of \texttt{IfcProduct}.
...
Regarding relationships between entities, IFC classes are generally structured in a deep hierarchies (is-a relationships), but some are organized in meronymic (part-of) trees, and there is support for spatial composition (fits-within) by means of \texttt{IfcSpatialStructureElements}\marginnote{IFC spatial structure element}\index{IFC spatial structure element}\marginnote{\faExternalLink\ \url{https://standards.buildingsmart.org/IFC/RELEASE/IFC4/ADD1/HTML/schema/ifcproductextension/lexical/ifcspatialstructureelement.htm}}.
...
In addition, property sets\marginnote{IFC property set}\index{IFC property set} can be used for attributes.
...
Properly georeferencing\marginnote{IFC georeferencing}\index{IFC georeferencing} an IFC file makes it possible to link the (local) coordinates inside an IFC model with their corresponding real-world coordinates, and thus to place the model of a single building or construction within the virtual environment.
...
\begin{table}
	...
	\begin{tabular}{m{1.6cm}m{2.2cm}m{5.5cm}}
		...
		LoGeoRef30 & Any Cartesian CRS, including projected coordinates (CRS not specified in the file) & \texttt{IfcCartesianPoint} referenced within \texttt{IfcSite} (defining the projected coordinates of the model reference point); \texttt{IfcDirection} attribute of \texttt{IfcSite}.\\ \addlinespace 
		LoGeoRef40 & Any Cartesian CRS, including projected coordinates (CRS not specified in the file) & \texttt{WorldCoordinateSystem} storing the coordinates of the reference point in any Cartesian CRS (including the projected ones) and the orientation in \texttt{TrueNorth}. Both are stored within \texttt{IfcGeometricRepresentationContext}. \\ \addlinespace 
...
%In the \texttt{LoGeoRef40} the stored direction represents the \texttt{TrueNorth}\marginnote{\faExternalLink\ \url{https://standards.buildingsmart.org/IFC/RELEASE/IFC2x2/FINAL/HTML/ifcrepresentationresource/lexical/ifcgeometricrepresentationcontext.html}} attribute, defined as \texttt{``Direction of the true north relative to the world coordinate system as established by the representation context''}.
%As also cited in the buildingSMART document about georeferencing\marginnote{\faExternalLink\ \url{https://www.buildingsmart.org/wp-content/uploads/2020/02/User-Guide-for-Geo-referencing-in-IFC-v2.0.pdf}}, this could be different from the grid north, considering a Cartesian system and possibly stored as part of the \texttt{IfcObjectPlacement} within \texttt{IfcSite} (inherited from the class \texttt{IfcSpatialStructureElement} and, in turn, \texttt{IfcProduct}), according to \texttt{LoGeoRef30}.
% As defined in the IFC specifications\marginnote{\faExternalLink\ \url{https://standards.buildingsmart.org/IFC/RELEASE/IFC2x/ADD1/HTML/ifcgeometricconstraintresource/lexical/ifcobjectplacement.html}} \emph{``The object placement can be given:
